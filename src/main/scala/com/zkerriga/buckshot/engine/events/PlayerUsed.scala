...
object PlayerUsed:
  def execute(state: GameState, used: PlayerUsed): V[DealerWins | ContinuableOutcome | GameState] =
    exec(state, used): table =>
      optional:
        updateBelief(used, conditionBelief(state.public, table, used, state.hidden.dealer.belief).?)

  def executeSimple(state: GameState, used: PlayerUsed): V[DealerWins | ContinuableOutcome | GameState] =
    exec(state, used): _ =>
      updateBelief(used, state.hidden.dealer.belief).some

  private def exec(state: GameState, used: PlayerUsed) =
